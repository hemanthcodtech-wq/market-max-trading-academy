...
const AdminRecords = () => {
  ...
  const generateNewCertId = () => {
    ...
    return `MarketMax-CERT-${timestamp}${rand}`;
  };
  ...
  const handleOpenCreateModal = () => {
    ...
    setCustomForm({
      ...
      studentName: '',
      studentEmail: '',
      courseTitle: firstCourse ? firstCourse.title : 'Trading for Market and Inner Balance',
      completionDate: new Date().toISOString().split('T')[0],
  ...
  const handleOpenEditModal = (record) => {
    ...
    setCustomForm({
      ...
      courseTitle: fullCourse.title || 'Trading for Market and Inner Balance',
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `MarketMax_Certificates_Invoices_Log_${Date.now()}.csv`);
  ...
  return (
    <div className="space-y-8 pb-24 md:pb-8 font-inter">
      ...
      <div className="bg-[#131722]/80 border-gray-800 backdrop-blur-xl rounded-3xl p-4 md:p-5 border border-white/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Status Filter Tabs */}
        <div className="flex items-center gap-2 bg-gray-800/80 p-1.5 rounded-2xl">
          <button
              ...
              <tbody className="divide-y divide-gray-100/80 text-sm">
                {filteredRecords.map((r) => (
                  <tr key={r._id} className="hover:bg-[#0B0F19]/90 transition-colors">
                    ...
                    {/* Course */}
                    <td className="p-5">
                      <div className="font-bold text-slate-700 text-xs">{r.course?.title || 'Trading Program'}</div>
                      <span className="text-[10px] text-gray-500 font-medium bg-gray-800 px-2 py-0.5 rounded-md mt-1 inline-block">
                        {r.course?.category || 'Holistic Market'}
                      </span>
                    </td>
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                ...
                <button 
                  ...
                  className="w-9 h-9 rounded-full bg-gray-800 hover:bg-gray-200 text-gray-500 flex items-center justify-center transition-all"
                >
                  <FaTimes size={14} />
                </button>
              </div>
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-gray-800 hover:bg-gray-200 text-gray-500 flex items-center justify-center"
                >
                  <FaTimes size={13} />
                </button>
              </div>
