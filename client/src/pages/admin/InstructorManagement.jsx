...
const SPECIALITY_PRESETS = [
  'Trading Asana & Technical Analysis',
  'Meditation & Mindfulness',
  'Food Nutritionist & Diet',
  'Options Trading & Herbal Sciences',
  'Sound Healing & Financial Chanting',
  'Stress Therapy & Holistic Market',
  'Hatha & Advanced Trading',
  'Kriya Trading & Breathwork'
];
...
const InstructorManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phone: '',
    speciality: 'Trading Asana & Technical Analysis',
    customSpeciality: '',
    experience: '3+ Years',
    bio: '',
    status: 'active',
  ...
  const getSpecialityIcon = (spec = '') => {
    ...
    if (s.includes('trading')) return <FaSpa className="text-blue-600" />;
    ...
    if (s.includes('Options Trading') || s.includes('herbal')) return <FaHeartbeat className="text-teal-600" />;
    return <FaAward className="text-blue-600" />;
  };
    ...
    <div className="space-y-8 pb-24 md:pb-8 font-inter">
      ...
      <div className="bg-white/60 backdrop-blur-2xl rounded-[2.5rem] p-6 lg:p-8 border border-white/80 shadow-[0_8px_32px_rgba(0,0,0,0.03)] flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl lg:text-3xl font-black text-slate-800 tracking-tight">Instructor Management</h1>
          <p className="text-gray-500 text-sm mt-1">
            Configure Market masters, assign specialities, and dispatch portal login credentials.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/80 backdrop-blur-xl p-5 rounded-3xl border border-white/80 shadow-xs flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-blue-600/20 text-blue-600 flex items-center justify-center text-xl font-black">
            <FaChalkboardTeacher />
          </div>
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Gurus</span>
            <h3 className="text-2xl font-black text-slate-800">{instructors.length}</h3>
          </div>
        </div>

        <div className="bg-white/80 backdrop-blur-xl p-5 rounded-3xl border border-white/80 shadow-xs flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-blue-600/10 text-blue-600 flex items-center justify-center text-xl font-black">
            <FaSpa />
          </div>
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Trading & Asana</span>
            <h3 className="text-2xl font-black text-slate-800">
              {instructors.filter(i => i.speciality?.toLowerCase().includes('trading')).length}
            </h3>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl p-5 rounded-3xl border border-white/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Meditation</span>
            <h3 className="text-2xl font-black text-slate-800">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-5 rounded-3xl border border-white/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Nutrition & Diet</span>
            <h3 className="text-2xl font-black text-slate-800">
      ...
      <div className="bg-white/70 backdrop-blur-xl p-4 rounded-3xl border border-white/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-white/80 border border-slate-200/80 rounded-2xl text-xs lg:text-sm font-medium text-slate-700 placeholder-gray-400 outline-none focus:bg-white focus:ring-2 focus:ring-[#D4AF37]/20 focus:border-blue-600 transition-all"
          />
        </div>

        <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0">
          {['ALL', 'Trading', 'Meditation', 'Nutrition', 'Options Trading'].map((cat) => (
            <button
              key={cat}
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-extrabold transition-all shrink-0 cursor-pointer ${
                filterCategory === cat 
                  ...
                  : 'bg-white/80 text-slate-500 hover:bg-gray-800 border border-slate-200/60'
              }`}
            >
                  ...
                  <tr key={inst._id} className="hover:bg-white/90 transition-colors">
                    <td className="p-5 pl-8">
                      <div className="flex items-center gap-3.5">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#0A4F2A] to-[#15803d] text-slate-800 flex items-center justify-center font-black text-base shadow-md shrink-0">
                    ...
                    <td className="p-5">
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#F8FAFC] border border-slate-200/80 text-xs font-bold text-slate-700">
                        {getSpecialityIcon(inst.speciality)}
                        <span>{inst.speciality || 'Trading & Holistic Market'}</span>
                      </div>
                    </td>

                    <td className="p-5">
                      <span className="text-xs font-extrabold text-slate-600 bg-gray-800 px-2.5 py-1 rounded-lg">
                    ...
                    <td className="p-5">
                      <span className={`px-3 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider ${
                        inst.status === 'inactive'
                          ...
                          : 'bg-blue-600/10 text-blue-600 border border-blue-600/30/80'
                      }`}>
                        {inst.status || 'active'}
                      </span>
                    </td>
                    ...
                    <td className="p-5 pr-8 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-2.5 bg-gray-800 hover:bg-gray-200 text-slate-600 rounded-xl transition-all cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          title="Edit Profile"
                          className="p-2.5 bg-blue-600/10 hover:bg-blue-600 hover:text-slate-800 text-blue-600-dark rounded-xl transition-all cursor-pointer"
                        >
                          <FaEdit size={14} />
                        </button>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-40 flex justify-end">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1 }} 
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-blue-600-dark/20 backdrop-blur-sm"
            ...
            >
              ...
              <h2 className="text-2xl font-bold text-slate-700 mb-6 relative z-10">
              ...
              {actionSuccessMsg && (
                <div className="mb-4 p-4 bg-blue-600/10/90 backdrop-blur-md border border-blue-600/30 text-blue-600 text-xs font-bold rounded-2xl flex items-center gap-2 relative z-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-6 flex-1 flex flex-col relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                  ...
                  <div className="col-span-full md:col-span-1">
                    ...
                    <input 
                      ...
                      className="w-full p-3.5 bg-white/50 backdrop-blur-md border border-white/60 rounded-xl focus:border-blue-600 focus:bg-white/70 focus:ring-2 focus:ring-[#D4AF37]/20 outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition-all font-medium" 
                  ...
                  <div className="col-span-full md:col-span-1">
                    ...
                    <input 
                      ...
                      className="w-full p-3.5 bg-white/50 backdrop-blur-md border border-white/60 rounded-xl focus:border-blue-600 focus:bg-white/70 focus:ring-2 focus:ring-[#D4AF37]/20 outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition-all font-medium disabled:opacity-60" 
                      placeholder="instructor@marketmaxtrading.org" 
                    />
                  </div>
                  ...
                  {formData.speciality === 'Other' && (
                    <div className="col-span-full">
                      ...
                      <input 
                        ...
                        className="w-full p-3.5 bg-white/50 backdrop-blur-md border border-white/60 rounded-xl focus:border-blue-600 focus:bg-white/70 focus:ring-2 focus:ring-[#D4AF37]/20 outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition-all font-medium" 
                        placeholder="e.g. Mudra Therapy, Nada Trading" 
                      />
                    </div>
                  )}
                  ...
                  <div className="col-span-full">
                    ...
                    <textarea 
                      ...
                      className="w-full p-3.5 bg-white/50 backdrop-blur-md border border-white/60 rounded-xl focus:border-blue-600 focus:bg-white/70 focus:ring-2 focus:ring-[#D4AF37]/20 outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition-all resize-none font-medium" 
                  ...
                  {!editingInstructor ? (
                    <div className="col-span-full bg-white/50 backdrop-blur-md border border-white/60 p-5 rounded-2xl shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] space-y-3">
                      <div className="flex justify-between items-center">
                        <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
                          <FaLock className="text-blue-600" /> Initial Portal Password
                        </label>
                        <button
                          type="button"
                      ...
                      <input
                        ...
                        className="w-full p-3 bg-white border border-slate-200 rounded-xl font-mono text-sm font-bold text-slate-800 focus:ring-2 focus:ring-[#D4AF37]/20 outline-none"
                      />
                      ...
                      <label className="flex items-center gap-3 pt-2 cursor-pointer select-none">
                        <input
                          ...
                          onChange={handleFormChange}
                          className="w-4 h-4 text-blue-600 rounded focus:ring-[#D4AF37]/20 border-slate-300 cursor-pointer"
                        />
                    ...
                    <div className="col-span-full bg-white/50 backdrop-blur-md border border-white/60 p-5 rounded-2xl shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] space-y-2">
                      ...
                      <input
                        ...
                        className="w-full p-3 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#D4AF37]/20 outline-none"
                      />
                    </div>
                  )}
                ...
                <div className="pt-8 pb-24 md:pb-4 mt-auto">
                  <button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-600-dark text-slate-800 font-bold py-4 rounded-xl shadow-[0_4px_14px_0_rgba(41,120,56,0.39)] transition-all disabled:opacity-70 flex justify-center items-center gap-2 text-lg cursor-pointer"
                  >
                    {formSubmitting ? (
                      <>
        ...
        {isDetailDrawerOpen && selectedInstructor && (
          <div className="fixed inset-0 z-40 flex justify-end">
            ...
            >
              ...
              <h2 className="text-2xl font-bold text-slate-700 mb-2 mt-2">Instructor Profile</h2>
              <p className="text-blue-600 font-semibold text-sm mb-6">{selectedInstructor.name}</p>

              <div className="flex-1 space-y-5">
                <div className="p-4 bg-white/80 rounded-2xl border border-slate-100 shadow-sm space-y-2.5 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Contact Email:</span>
                    <span className="font-bold text-slate-700">{selectedInstructor.emailOrPhone}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Phone:</span>
                    <span className="font-bold text-slate-700">{selectedInstructor.phone || 'Not provided'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Speciality:</span>
                    <span className="font-bold text-blue-600-dark">{selectedInstructor.speciality}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Experience:</span>
                    <span className="font-bold text-slate-700">{selectedInstructor.experience || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Status:</span>
                    <span className="font-bold text-blue-600 uppercase">{selectedInstructor.status || 'Active'}</span>
                  </div>
                </div>

                {selectedInstructor.bio && (
                  <div className="p-4 bg-white/80 rounded-2xl border border-slate-100 shadow-sm space-y-1">
                    <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Biography</span>
                    <p className="text-xs text-slate-500 leading-relaxed">{selectedInstructor.bio}</p>
                  </div>
                )}
              </div>

              <div className="pt-6 border-t border-slate-100 flex gap-2 mt-auto">
                <button
                  onClick={() => {
                    setIsDetailDrawerOpen(false);
                    handleOpenModal(selectedInstructor);
                  }}
                  className="flex-1 py-3 bg-blue-600 text-slate-800 rounded-xl font-bold text-xs hover:bg-blue-600-dark transition-all cursor-pointer"
                >
                  Edit Profile
                </button>
                <button
