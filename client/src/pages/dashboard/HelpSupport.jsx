...
const HelpSupport = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: "Yes! Upon successfully completing your 1-month live course attendance and practice, an official verified certificate from MarketMax Trading Academy will be generated in your 'Certificates' tab."
    },
    {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-24 md:pb-12 px-4 md:px-0 font-inter">
      ...
      <div className="flex items-center mb-6 mt-2 md:hidden">
        ...
        <h1 className="text-xl font-bold text-[#D4AF37]-dark">Help & Support</h1>
      </div>

      {/* Desktop Header */}
      <div className="hidden md:block mb-6">
        <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">Help & Support</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <span className="mt-3 text-xs font-bold text-[#1e9e4d] bg-[#1E293B] px-3 py-1 rounded-full shadow-xs">Chat Now &rarr;</span>
        </a>

        {/* Email Support */}
        <a
          href="mailto:support@marketmaxtrading.org"
          className="bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 border border-[#D4AF37]/30 p-6 rounded-3xl flex flex-col items-center text-center transition-all group shadow-sm"
        >
          <div className="w-14 h-14 bg-[#D4AF37] text-white rounded-2xl flex items-center justify-center text-2xl mb-3 shadow-md group-hover:scale-110 transition-transform">
            <FaEnvelope />
          </div>
          <h3 className="font-bold text-white text-base">Email Support</h3>
          <p className="text-xs text-gray-500 mt-1">support@marketmaxtrading.org</p>
          <span className="mt-3 text-xs font-bold text-[#D4AF37] bg-[#1E293B] px-3 py-1 rounded-full shadow-xs">Send Email &rarr;</span>
        </a>
        ...
        >
          ...
          <h3 className="font-bold text-white text-base">Helpline Hours</h3>
          <p className="text-xs text-gray-500 mt-1">Mon - Sat: 6:00 AM - 8:00 PM</p>
          <span className="mt-3 text-xs font-bold text-amber-700 bg-[#1E293B] px-3 py-1 rounded-full shadow-xs inline-block">+91 9640275275</span>
        </a>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="space-y-3">
            {faqs.map((faq, index) => {
              ...
              return (
                <div key={index} className="bg-[#1E293B] rounded-2xl border border-gray-700/80 overflow-hidden shadow-xs">
                  <button
                    onClick={() => setActiveFaq(isOpen ? null : index)}
                    className="w-full px-5 py-4 text-left font-bold text-gray-100 flex justify-between items-center gap-4 hover:bg-[#0F172A]/70 transition-colors"
                  >
                    <span className="text-sm md:text-base">{faq.q}</span>
                    <FaChevronDown className={`text-gray-400 text-xs transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#D4AF37]' : ''}`} />
                  </button>
                  {isOpen && (
                    <motion.div
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: 'auto' }}
                      className="px-5 pb-5 text-sm text-gray-400 leading-relaxed border-t border-gray-800 pt-3"
                    >
                      {faq.a}
                    </motion.div>
                  )}
        ...
        <div className="lg:col-span-2">
          <div className="bg-[#1E293B] rounded-3xl border border-gray-700/80 p-6 md:p-8 shadow-sm">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-700 bg-[#0F172A] focus:bg-[#1E293B] focus:border-[#D4AF37] outline-none text-sm"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-300 mb-1">Email or Phone</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-300 mb-1">Query Topic</label>
                  <select
                    value={formData.queryType}
                    onChange={(e) => setFormData({ ...formData, queryType: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-700 bg-[#0F172A] focus:bg-[#1E293B] focus:border-[#D4AF37] outline-none text-sm cursor-pointer"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-300 mb-1">Message</label>
                  <textarea
                    required
                    rows="4"
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-700 bg-[#0F172A] focus:bg-[#1E293B] focus:border-[#D4AF37] outline-none text-sm resize-none"
                  ></textarea>
                </div>

                <button
                  type="submit"
                  className="w-full py-3 bg-[#D4AF37] hover:bg-[#D4AF37]-dark text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-all shadow-[0_4px_12px_rgba(41,120,56,0.3)]"
                >
