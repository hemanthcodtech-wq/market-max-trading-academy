...
const Certificates = () => {
  ...
  const handleDownloadPDF = async (enrollment) => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Certificate-${enrollment.certificateId || 'MarketMax-Completion'}.pdf`);
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] flex flex-col px-4 sm:px-6 lg:px-8 py-8 font-inter pb-24 md:pb-12">
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-24 right-6 z-50 bg-[#D4AF37] text-white px-5 py-3 rounded-2xl shadow-xl font-bold text-sm flex items-center gap-2"
          >
      ...
      <div className="flex items-center justify-between mb-8 max-w-5xl mx-auto w-full">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="p-2 rounded-xl bg-[#1E293B] text-[#D4AF37] hover:bg-[#0F172A] border border-gray-700 transition-colors shadow-xs cursor-pointer">
            <FaArrowLeft size={16} />
          </button>
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">Official Certificates Portal</h1>
      ...
      <div className="max-w-5xl mx-auto w-full space-y-8">
        ...
        {completedEnrollments.length > 0 ? (
                  ...
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl transition-all border flex items-center justify-between cursor-pointer ${
                      selectedCertIndex === idx 
                        ? 'bg-[#D4AF37] text-white border-[#D4AF37] shadow-md shadow-brand-green/20' 
                        : 'bg-[#1E293B] text-gray-100 border-gray-700/80 hover:bg-[#0F172A]'
                    }`}
                  >
                    <div className="min-w-0 pr-2">
                      <h4 className="font-extrabold text-sm truncate">{enr.course?.title || 'Trading Program'}</h4>
                      <p className={`text-xs mt-0.5 ${selectedCertIndex === idx ? 'text-green-100' : 'text-gray-400'}`}>
                        {enr.certificateId || `ID: MarketMax-CERT-${enr._id.slice(-6).toUpperCase()}`}
                      </p>
                    </div>
                    <FaAward className={selectedCertIndex === idx ? 'text-yellow-300' : 'text-[#D4AF37]'} size={22} />
                  </button>
                ))}
              </div>
              ...
              {inProgressEnrollments.length > 0 && (
                <div className="pt-4 border-t border-gray-700/60 space-y-3">
                  ...
                  {inProgressEnrollments.map((inEnr) => (
                    <div key={inEnr._id} className="bg-[#1E293B]/80 rounded-2xl p-4 border border-gray-700/60 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-xs text-gray-100 truncate max-w-[190px]">{inEnr.course?.title}</span>
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-[#0F172A] hover:bg-[#D4AF37]/10 text-[#D4AF37] text-xs font-bold rounded-xl border border-gray-700 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                      >
                ...
                <div className="w-full space-y-5">
                  ...
                  <div className="flex flex-wrap gap-3 w-full">
                    ...
                    <button
                      ...
                      className="px-5 py-4 bg-[#1E293B] border border-gray-600 hover:bg-[#0F172A] text-gray-100 font-extrabold rounded-2xl text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-xs cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 py-4 bg-[#D4AF37] hover:bg-[#D4AF37]-dark text-white font-extrabold rounded-2xl shadow-lg shadow-brand-green/20 hover:shadow-brand-green/40 transition-all duration-300 flex items-center justify-center gap-2 text-xs sm:text-sm disabled:opacity-70 cursor-pointer"
                    >
                      {downloading ? (
                        <>
                    ...
                    {activeCert.certificateUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-4 py-4 bg-[#1E293B] border border-gray-700 text-gray-300 hover:bg-[#0F172A] font-bold rounded-2xl text-xs sm:text-sm flex items-center justify-center gap-2 transition-colors shadow-xs"
                      >
          ...
          /* Empty State */
          <div className="bg-[#1E293B]/80 rounded-3xl p-10 md:p-14 border border-white shadow-sm text-center max-w-xl mx-auto space-y-6">
            <div className="w-20 h-20 bg-[#D4AF37]/10 text-[#D4AF37] rounded-full flex items-center justify-center mx-auto text-3xl">
              <FaAward />
            </div>
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-white">No Certificates Earned Yet</h2>
            ...
            {inProgressEnrollments.length > 0 ? (
              <div className="space-y-3 pt-2 text-left">
                ...
                {inProgressEnrollments.map((enr) => (
                  <div key={enr._id} className="p-4 bg-[#0F172A] rounded-2xl border border-gray-700 flex items-center justify-between">
                    <div>
                      <h5 className="font-bold text-sm text-gray-100">{enr.course?.title}</h5>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-[#D4AF37] hover:bg-[#D4AF37]-dark text-white text-xs font-bold rounded-xl transition-all shadow-xs cursor-pointer"
                    >
                      Go to Classes →
                    </button>
                  </div>
              ...
              <button
                ...
                className="px-8 py-3.5 bg-[#D4AF37] hover:bg-[#D4AF37]-dark text-white font-bold text-sm rounded-2xl shadow-md transition-all inline-flex items-center gap-2 cursor-pointer"
              >
        ...
        {isEditNameOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative bg-[#1E293B] w-full max-w-md rounded-3xl p-6 md:p-8 shadow-2xl z-10 space-y-5"
            >
              <div className="flex items-center justify-between border-b border-gray-800 pb-3">
                <h3 className="font-black text-lg text-white flex items-center gap-2">
                  <FaUserCheck className="text-[#D4AF37]" /> Correct Printed Legal Name
                </h3>
                <button 
                  onClick={() => !savingName && setIsEditNameOpen(false)}
                  className="p-2 text-gray-400 hover:text-gray-400 rounded-full cursor-pointer"
                >
                  <FaTimes size={14} />
                </button>
              </div>
              ...
              <form onSubmit={handleSaveName} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full p-3.5 bg-[#0F172A] border border-gray-700 rounded-xl focus:bg-[#1E293B] focus:ring-2 focus:ring-[#D4AF37]/20 outline-none font-bold text-white"
                  />
                </div>

                <div className="p-3 bg-[#D4AF37]/10 rounded-xl border border-[#D4AF37]/30 text-emerald-900 text-[11px] leading-relaxed">
                ...
                <div className="flex items-center justify-end gap-3 pt-3">
                  <button 
                    ...
                    className="px-5 py-2.5 rounded-xl bg-gray-800 hover:bg-gray-200 text-gray-300 font-bold text-xs cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit" 
                    disabled={savingName}
                    className="px-6 py-2.5 rounded-xl bg-[#D4AF37] hover:bg-[#D4AF37]-dark text-white font-extrabold text-xs shadow-md disabled:opacity-60 flex items-center gap-2 cursor-pointer"
                  >
                    {savingName ? (
