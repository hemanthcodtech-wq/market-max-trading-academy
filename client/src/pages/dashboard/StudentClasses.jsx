          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
              ...
              <div className="lg:sticky lg:top-28 space-y-4">
                ...
                <div className="bg-white/[0.055] backdrop-blur-xl rounded-2xl p-6 md:p-8 flex flex-col justify-center text-white shadow-[0_20px_45px_rgba(0,0,0,0.25)] border border-white/10 relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 mb-8">
                    <div className="w-12 h-12 border border-white/20 rounded-xl bg-[#1E293B]/10 flex items-center justify-center backdrop-blur-md shadow-sm">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-1 gap-3">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] bg-[#1E293B]/20 px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider">Join Group →</span>
                        </div>
                  ...
                  <button 
                    ...
                    disabled={completing}
                    className="rounded-xl py-4 px-5 flex items-center justify-between shadow-sm border border-[#D4AF37]/30 bg-green-50 hover:bg-green-100 text-[#D4AF37] transition-all duration-300 col-span-2 lg:col-span-1"
                  >
                    ...
                    <FaChevronRight className="text-[#D4AF37] text-[12px]" />
                  </button>
                </div>
              </div>
              ...
              <div className="lg:col-span-2">
                <div className="bg-[#1E293B]/40 md:bg-transparent rounded-2xl md:rounded-none p-2 md:p-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {activeTab === 'classes' && (
                      allClasses.length > 0 ? allClasses.map((cls, index) => {
                            ...
                            <div className="flex items-start gap-4 mb-5">
                              ...
                              <div className="flex flex-col pt-1 flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {isPast && (
                                    <span className="text-[10px] font-bold text-gray-500 bg-gray-800 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                      COMPLETED
                                    </span>
                                  )}
                                </div>
                                <span className="text-base font-black text-gray-100 line-clamp-2 leading-tight">{cls.title || 'Live Class Session'}</span>
                              </div>
                            </div>
                              ...
                              {isToday ? (
                                cls.zoomLink ? (
                                  <button 
                                    ...
                                    className="text-[11px] font-bold text-white tracking-wider uppercase bg-[#D4AF37] hover:bg-[#D4AF37]-dark px-4 py-2 rounded-full shadow-md shadow-brand-green/30 transition-all hover:scale-105 flex items-center gap-1.5 cursor-pointer"
                                  >
                                    <FaVideo size={12}/> JOIN LIVE
                                  </button>
                                ) : (
                                  <span className="text-[11px] font-bold text-gray-400 bg-gray-800 px-3 py-1.5 rounded-full">Link Pending</span>
                                )
                              ) : isPast ? (
                                <button 
                                  onClick={() => handleViewClassMaterial(cls)} 
                                  className="text-[11px] font-bold text-[#D4AF37]-dark hover:text-white bg-[#D4AF37]/10 hover:bg-[#D4AF37] px-4 py-2 rounded-full border border-[#D4AF37]/20 transition-all flex items-center gap-1.5 shadow-sm"
                                >
                        ...
                        <div className="col-span-full text-center p-12 bg-[#1E293B] rounded-3xl border border-dashed border-gray-600 text-gray-500 font-medium">
                    ...
                    {activeTab === 'materials' && (
                      materials.length > 0 ? materials.map((mat, index) => (
                        <motion.div 
                          initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
                          key={mat._id} 
                          className="bg-[#1E293B] rounded-[20px] p-5 flex flex-col justify-between border border-gray-700 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-lg transition-all duration-300 group"
                        >
                          <div className="flex items-start gap-4 mb-6">
                            <div className="w-12 h-12 rounded-xl bg-[#0F172A] flex items-center justify-center border border-gray-800 shrink-0 group-hover:scale-110 transition-transform">
                              {mat.materialType === 'Recording' ? <FaVideo className="text-red-500" size={18} /> : <FaFilePdf className="text-blue-500" size={18} />}
                            </div>
                            <div className="flex flex-col pt-1">
                              <span className="text-base font-black text-gray-100 line-clamp-2 leading-tight">{mat.topicsCovered}</span>
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-auto">
                            ...
                            <button 
                              ...
                              className="text-[11px] font-bold text-white tracking-wider uppercase bg-[#D4AF37] hover:bg-[#D4AF37]-dark px-4 py-2 rounded-full shadow-sm transition-colors"
                            >
                              VIEW
                            </button>
                          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#1E293B] rounded-3xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col relative z-10 overflow-hidden">
              <div className="bg-[#1E293B] px-6 py-5 border-b border-gray-800 flex justify-between items-center shrink-0">
                <div>
                  <h3 className="font-bold text-lg text-white">{selectedMaterial.topicsCovered}</h3>
      ...
      <AnimatePresence>
        {noMaterialNotice && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#1E293B] rounded-3xl shadow-2xl w-full max-w-md p-8 relative z-10 text-center">
              ...
              <h3 className="font-bold text-xl text-white mb-2">Class Materials Notice</h3>
              ...
              <button 
                ...
                className="w-full py-3 bg-[#D4AF37] hover:bg-[#D4AF37]-dark text-white font-bold rounded-xl transition-all shadow-md"
              >
                Got It
              </button>
            </motion.div>
