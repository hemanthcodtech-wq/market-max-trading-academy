...
import { 
  ...
  FaSignOutAlt, FaExternalLinkAlt, FaChartLine, FaBars, FaTimes,
...
const InstructorLayout = () => {
  ...
  const instructor = rawUser ? JSON.parse(rawUser) : { name: 'Trading Instructor', speciality: 'Trading Master' };
    ...
    <div className="flex h-screen bg-[#0B0F19] font-inter overflow-hidden relative text-gray-300">
      ...
      <aside className="hidden md:flex md:static inset-y-0 left-0 w-64 lg:w-72 bg-[#0F172A]/90 backdrop-blur-2xl text-gray-300 flex-col shadow-[4px_0_30px_rgba(0,0,0,0.5)] z-30 shrink-0 border-r border-gray-800">
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-800 bg-[#0B0F19]/40 sticky top-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold text-sm tracking-wider text-white">INSTRUCTOR PANEL</span>
              <span className="text-[10px] font-semibold text-[#D4AF37] uppercase tracking-widest">Faculty Portal</span>
            </div>
          </div>
        </div>

        {/* Instructor Mini Badge */}
        <div className="p-4 mx-4 my-4 bg-[#1E293B] border border-gray-700 rounded-2xl shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#D4AF37]/20 border border-[#D4AF37]/30 text-[#D4AF37] flex items-center justify-center font-black text-sm shadow-xs">
              {instructor.name ? instructor.name.charAt(0).toUpperCase() : <FaChartLine />}
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-bold text-xs text-white truncate">{instructor.name || 'Instructor'}</div>
              <div className="text-[11px] text-gray-400 font-bold truncate">{instructor.speciality || 'Trading Expert'}</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
              ...
              className={({ isActive }) =>
                `flex items-center gap-3.5 px-4 py-3.5 rounded-2xl transition-all duration-300 ${
                  isActive 
                    ...
                    : 'text-gray-400 hover:bg-[#1E293B] hover:text-[#D4AF37] font-semibold hover:border-gray-700 border border-transparent'
                }`
              }
            >
              {({ isActive }) => (
          ...
          <div className="pt-6 px-3 mb-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Quick Access</div>
          <Link
            to="/"
            target="_blank"
            rel="noreferrer"
        ...
        <div className="p-4 border-t border-gray-800 bg-[#0B0F19]/40">
          <button 
            ...
            className="flex items-center gap-3 px-4 py-3 w-full rounded-2xl text-red-400 hover:bg-red-500/10 hover:text-red-500 font-bold border border-transparent hover:border-red-500/20 transition-all text-sm cursor-pointer"
          >
        ...
        <header className="h-16 md:h-20 bg-[#0F172A]/80 backdrop-blur-xl border-b border-gray-800 flex items-center justify-between px-4 md:px-8 z-20 shrink-0 shadow-lg sticky top-0">
          ...
          <div className="hidden md:flex items-center gap-3">
            <h2 className="text-base lg:text-lg font-extrabold text-white tracking-tight">
              MarketMax Trading Academy • Instructor Workspace
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2.5 sm:gap-4">
            ...
            >
              ...
              <div className="hidden sm:flex flex-col">
                <span className="text-xs font-bold text-white leading-tight truncate max-w-[120px]">{instructor.name || 'Instructor'}</span>
                <span className="text-[10px] font-semibold text-gray-400 truncate max-w-[120px]">{instructor.speciality || 'Faculty'}</span>
              </div>
            </div>
          </div>
        </header>
            ...
            >
              <div className="flex items-center justify-between pb-4 border-b border-gray-800 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col">
                    <span className="font-black text-xs text-white">INSTRUCTOR PANEL</span>
                    <span className="text-[9px] text-[#D4AF37] uppercase font-bold">Faculty Hub</span>
                  </div>
                </div>
                <button 
                  onClick={() => setMobileMenuOpen(false)}
                  className="w-8 h-8 rounded-full bg-[#1E293B] border border-gray-700 flex items-center justify-center text-gray-400 hover:text-white"
                >
                  <FaTimes size={13} />
                </button>
              </div>
              ...
              <div className="p-4 bg-[#1E293B] border border-gray-700 rounded-2xl mb-5 space-y-2 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="font-extrabold text-sm text-white truncate">{instructor.name || 'Instructor'}</div>
                    <div className="text-xs text-gray-400 font-bold truncate">{instructor.speciality || 'Trading Expert'}</div>
                  </div>
                </div>
              ...
              <div className="pt-4 border-t border-gray-800">
                <button
                  onClick={handleLogout}
                  className="w-full py-3 px-4 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-bold text-xs rounded-xl border border-red-500/20 flex items-center justify-center gap-2 transition-all cursor-pointer"
                >
                  <FaSignOutAlt />
                  <span>Sign Out</span>
                </button>
